...
for epoch in range(opt.n_epochs):
    ...
    # plt.plot(data[:, 0].cpu().numpy(), data[:, 1].cpu().numpy(), '.b')

    z = Variable(Tensor(np.random.normal(0, 1, (8000, opt.latent_dim))))
...
# z = Variable(Tensor(np.random.normal(0, 1, (2000, opt.latent_dim))))
# gen_data = generator(z)
# plt.plot(data[:, 0].numpy(), data[:, 1].numpy(), '.b', gen_data[:, 0].detach().numpy(), gen_data[:, 1].detach().numpy(), '.r')
# plt.show()
