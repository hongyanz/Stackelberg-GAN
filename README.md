...
generators, where each row corresponds to one generator. When there is only one generator, the digits are not very diverse with many "1"’s and much fewer "2"’s. As the number of generators increases, the images tend to be more diverse. In particular, for 10-generator
